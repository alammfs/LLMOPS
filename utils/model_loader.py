...
class ApiKeyManager:
    ...
    def __init__(self):
        ...
        for key in self.REQUIRED_KEYS:
            if not self.llm_api_keys.get(key):
                env_val = os.getenv(key)
                if env_val:
                    self.llm_api_keys[key] = env_val
                    log.info(f"Loaded {key} from individual env var")

        # Final check
        missing = [k for k in self.REQUIRED_KEYS if not self.llm_api_keys.get(k)]
        if missing:
            log.error("Missing required API keys", missing_keys=missing)
            raise DocumentPortalException("Missing API keys", sys)

        log.info("API keys loaded", keys={k: v[:6] + "..." for k, v in self.llm_api_keys.items()})


    def get(self, key: str) -> str:
        val = self.llm_api_keys.get(key)
        if not val:
